...
**`tab-bar`** — Barra inferior fija de 56px con 4 iconos: Planificador (icono calendario), Recetas (icono libro), Compra (icono carrito), Familia (icono personas). Fondo {colors.surface}, borde superior 1px {colors.hairline}. Icono + label en {typography.caption}.

**`tab-bar-active`** — Tab seleccionado: icono y label en {colors.primary}.
...
**`search-bar`** — Barra de búsqueda pill en la parte superior. Fondo {colors.surface-warm}, icono de lupa a la izquierda. Al enfocar cambia a {colors.surface} con borde 1px {colors.primary}.

**`filter-chip`** / **`filter-chip-active`** — Chips horizontales scrollables debajo del search bar. "Desayuno", "Sin gluten", "Alta en proteína", etc. Inactivo: fondo warm, texto body. Activo: fondo primary, texto blanco.

**`recipe-card`** — Tarjeta de receta en el catálogo. Fondo blanco, rounded {rounded.lg}, padding 16px. Contiene: nombre del plato, etiquetas de tipo de comida y compatibilidad como badges, indicador nutricional. Sin foto en MVP.
...
- **Don't** introducir fotos de platos en el MVP. Las fotos dominan la atención y compiten con la estructura funcional del planificador.
