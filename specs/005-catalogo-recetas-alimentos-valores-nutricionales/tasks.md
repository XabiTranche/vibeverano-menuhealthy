...
- [X] T001 Initialize React Native project with Expo + TypeScript strict + WatermelonDB + SQLCipher + Supabase + react-i18next (shared infrastructure — skip if already done)
...
- [X] T002 Configure Jest + React Native Testing Library
...
- [X] T003 [P] Configure ESLint + Prettier with project rules
...
- [X] T004 Create WatermelonDB schema for spec 005 tables (MasterIngredient, BaseCatalogRecipe, FamilyRecipe, RecipeIngredient, CompatibilityTag, GoalTag) in `src/database/schema.ts`
...
- [X] T005 [P] Create seed data: `src/database/seeds/master-ingredients.json` (~500 ingredients with canonicalName, synonyms, category, nutritionalPer100g, allergenFlags)
...
- [X] T006 [P] Create seed data: `src/database/seeds/base-recipes.json` (~50 recipes with all fields)
...
- [X] T007 Implement seed loading mechanism (first-run insert from bundled JSON to WatermelonDB)
...
- [X] T008 [P] Configure Supabase tables + RLS policies for spec 005 entities
...
- [X] T009 [P] Setup i18n namespace `005-catalog` with es.json translations
...
- [X] T010 [P] [US1] Create MasterIngredient model in `src/models/MasterIngredient.ts`
...
- [X] T011 [P] [US1] Create FamilyRecipe model in `src/models/FamilyRecipe.ts`
...
- [X] T012 [P] [US1] Create RecipeIngredient model in `src/models/RecipeIngredient.ts`
...
- [X] T013 [US1] Implement IngredientService in `src/services/IngredientService.ts` (search, getById, getByCategory, getAllCategories)
...
- [X] T014 [US1] Implement RecipeService.createRecipe + updateRecipe + deleteRecipe in `src/services/RecipeService.ts`
...
- [X] T018 [US2] Implement RecipeService.searchRecipes (text search Q.like + filters) in `src/services/RecipeService.ts`
...
- [X] T019 [US2] Implement SearchBar component in `src/components/SearchBar.tsx`
...
- [X] T020 [US2] Implement filter chips UI (mealType, compatibilityTag, goalTag) in `src/components/FilterChips.tsx`
...
- [X] T021 [US2] Implement RecipeCatalog screen in `src/screens/RecipeCatalog/` (list + search + filters + contextual pre-filter from planner)
...
- [X] T022 [US3] Implement RecipeService.getByMealType in `src/services/RecipeService.ts`
...
- [X] T023 [US3] Implement MealTypeFilter component + section headers in RecipeCatalog screen
...
- [X] T024 [US4] Implement RecipeService.getBaseCatalog + getBaseCatalogByMealType + copyFromBase in `src/services/RecipeService.ts`
